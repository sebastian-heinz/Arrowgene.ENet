...
{
    public class ENetCompressor : IENetCompressor
    {
        /*
         * adaptation constants tuned aggressively for small packet sizes rather than large file compression
         */
        public static uint ENET_RANGE_CODER_TOP = 1 << 24;
        public static uint ENET_RANGE_CODER_BOTTOM = 1 << 16;

        public static uint ENET_CONTEXT_SYMBOL_DELTA = 3;
        public static uint ENET_CONTEXT_SYMBOL_MINIMUM = 1;
        public static uint ENET_CONTEXT_ESCAPE_MINIMUM = 1;

        public static uint ENET_SUBCONTEXT_ORDER = 2;
        public static uint ENET_SUBCONTEXT_SYMBOL_DELTA = 2;
        public static uint ENET_SUBCONTEXT_ESCAPE_DELTA = 5;

        public object Context { get; }


        // uint enet_range_coder_compress(void context, const ENetBuffer* inBuffers, size_t inBufferCount, size_t inLimit, enet_uint8 * outData, size_t outLimit)
        public uint Compress(ENetBuffer[] inBuffers, uint bufferIndex, uint inBufferCount, uint inLimit,
            out byte[] outData, uint outLimit)
        {
            /* Start
      ...
      */
        ...
        public uint Decompress(byte inData, uint inLimit, out byte[] outData, uint outLimit)
        {
            /*
            ENetRangeCoder* rangeCoder = (ENetRangeCoder*) context;
            enet_uint8* outStart = outData,  *outEnd = &outData[outLimit];
...
//        ENET_CONTEXT_DECODE(context, symbol_, code, value_, under_, count_, update, minimum, return 0, exclude (node ->
//        value, after, before), return 0, return 0)
